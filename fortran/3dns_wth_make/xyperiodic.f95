...
real ( kind = 8 ), DIMENSION(Ny,Nx,Nz+1):: temp,turbine
...
vdudy,vdudyold,vdvdy,vdvdyold,vdwdy,vdwdyold,unew,vnew,wnew,contcont,savearray,th
...
INTEGER(kind=4)      :: rank,idist,odist,istride,ostride,howmany,buffer
...
call dfftw_plan_many_dft(planf,rank,ppp,howmany,inf,inembed,istride,idist,outf, onembed,ostride,odist, FFTW_FORWARD, FFTW_ESTIMATE);
call dfftw_plan_many_dft(planb,rank,ppp,howmany,outf,inembed,istride,idist,inf, onembed,ostride,odist, FFTW_BACKWARD, FFTW_ESTIMATE);
...
!call dfftw_execute_dft(planf,turb,th);

! Buffer logspace
buffer=floor((REAL(Nx)/15));
!open(12, file = 'alfa.dat', form = 'unformatted');  read(12) alfa;  close(12)
...
! Y DERIVATIVES IN FOURIER SPACE du/dy&dv/dy&dw/dy at time t & t-1
do l=1,Ny; 
...
!------------------------------------------------------------------------------------------------------------------------------------------!
!------------------------------------------------------------------------------------------------------------------------------------------!
!Z DERIVATIVES IN REAL SPACE WITH CHEBYSHEV DIFF. MATRICES FOR NON LINEAR TERMS 
do l=1,Nx;do m=1,Ny;
...
!---------------------------loop nr.23 for X modes & loop nr.36 for Y modes  --------------------------------------------------------------!
!------------------------------------------------------------------------------------------------------------------------------------------!
!------------------------------------------------------------------------------------------------------------------------------------------!
...
! Construct LHS for intermediate velocities
...
! Merge all RHSs in one matrix-BB for simultaneous Gaussian Elimination
...
! CONSTRUCT THE SYSTEM FOR PRESSURE LHS & RHS
...
!impose boundaries.
u(:,:,Nz+1)=1;u(:,:,1)=1;v(:,:,Nz+1)=0;v(:,:,1)=0;w(:,:,Nz+1)=0;w(:,:,1)=0;

!Buffer zine implement
!do l=1,buffer;unew(:,Nx-buffer+l-1,:)=unew(:,Nx-buffer+l-1,:)*(1-alfa(l))+Uinf(:,l,:)*alfa(l);end do
